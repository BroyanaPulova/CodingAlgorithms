...
string ShannonFano::Encode()    //кодиране
{
	tree = new Node();

	fTable = EncodeHelper::CalculateFrequency(text);   //изчислява честотата на символите      
	BuildTree(tree, fTable, 100);                      //разпределя миволите в дървото
	codeEntity = EncodeHelper::GenerateCodeTable(tree);                           //създава "таблица" със символите и кодовете
